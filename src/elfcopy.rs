...
fn copy_file<'data, Elf: ElfType<Endian = Endianness>>(
    ...
    let in_elf = Elf::File::parse(in_data)?;
    ...
    for (i, in_section) in in_sections.iter().enumerate() {
        ...
        match in_section.sh_type(endian) {
            ...
            | elf::SHT_INIT_ARRAY
            | elf::SHT_FINI_ARRAY => {
                let section_name = in_sections.section_name(endian, in_section)?;
                name = Some(writer.add_section_name(section_name));
                index = writer.reserve_section_index();

                // TODO: sigstore
                if in_section.sh_type(endian) == elf::SHT_NOTE
                    && String::from_utf8_lossy(section_name) == SIGNATURE_V1_SECTION
                {
                    in_signatures = Some(index);
                }
            }
            elf::SHT_STRTAB => {
                if i == in_syms.string_section().0 {
                    ...
                    bail!("Unsupported string section {}", i);
                }
            }
            elf::SHT_SYMTAB => {
                if i == in_syms.section().0 {
                    ...
                    bail!("Unsupported symtab section {}", i);
                }
            }
            elf::SHT_SYMTAB_SHNDX => {
                if i == in_syms.shndx_section().0 {
                    ...
                    bail!("Unsupported symtab shndx section {}", i);
                }
            }
            elf::SHT_DYNSYM => {
                if i == in_dynsyms.section().0 {
                    ...
                    bail!("Unsupported dynsym section {}", i);
                }
            }
            elf::SHT_DYNAMIC => {
            ...
            other => {
                bail!("Unsupported section type {:x}", other);
            }
        }
    ...
    // TODO: sigstore
    // check sigstore section
    match in_signatures {
        Some(signatures) => {
            // FIXME: already have one
            // TODO: parse existing
        }
        None => {
            // need to add one
            let name = Some(writer.add_section_name(SIGNATURE_V1_SECTION.as_bytes()));
        ...
        for d in in_dynamic {
            ...
            let string = if d.is_string(endian) {
                let s = in_dynamic_strings
                    .get(val.try_into()?)
                    .map_err(|_| anyhow!("Invalid dynamic string"))?;
    ...
    if in_segments.is_empty() {
        ...
        for (i, in_section) in alloc_sections.iter() {
            ...
            match in_section.sh_type(endian) {
                ...
                other => {
                    bail!("Unsupported alloc section index {}, type {}", *i, other);
                }
            }
        }
    ...
    // TODO: sigstore
    if in_signatures.is_none() {
        let i = in_sections.len();
        out_sections[i].offset = writer.reserve(sig_data.len(), SIGNATURE_V1_SECTION_ALIGN);
    }
    ...
    if in_segments.is_empty() {
        ...
        for (i, in_section) in alloc_sections.iter() {
            ...
            match in_section.sh_type(endian) {
                ...
                other => {
                    bail!("Unsupported alloc section type {:x}", other);
                }
            }
        }
    ...
    if in_signatures.is_none() {
        let i = in_sections.len();
        writer.write_align(SIGNATURE_V1_SECTION_ALIGN);
        debug_assert_eq!(out_sections[i].offset, writer.len());
        // write data
        writer.write(&sig_data);
    }
    ...
    if matches!(in_signatures, None) {
        ...
        writer.write_section_header(&object::write::elf::SectionHeader {
            ...
            sh_addr: 0,
            sh_offset: out_section.offset as u64,
            sh_size: sig_data.len() as u64,
            sh_link: 0, // FIXME: need to check if we need that
            sh_info: 0, // FIXME: need to check if we need that
            sh_addralign: SIGNATURE_V1_SECTION_ALIGN as u64,
            sh_entsize: 0,
        });
    }
